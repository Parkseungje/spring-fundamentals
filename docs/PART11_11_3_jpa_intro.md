...
### 생태계 (실무 표준 조합)
- **Spring Data JPA**: 위처럼 리포지토리 인터페이스 기반 자동화.
- **Querydsl**: 타입 안전한 동적 쿼리(컴파일 시점 오류 검출). 복잡·동적 쿼리에 쓴다.
- 실무 표준: **Spring Data JPA + Querydsl** 조합. (단순 CRUD는 Data JPA, 복잡 쿼리는 Querydsl)

---

## 2. 실습으로 확인하기

> - **가설**: SQL을 한 줄도 안 쓰고도 save/findById/findByName이 동작한다 — JPA(Hibernate)가 SQL을 대신 생성하므로.

### 코드 (`com.study.part11_jpa.s03_jpa_intro`)
- `Item` — `@Entity`로 매핑한 최소 엔티티.
- `ItemRepository` — `JpaRepository` 상속 인터페이스(+ 쿼리 메서드 findByName, findByPriceGreaterThanEqualOrderByPriceAsc).
- 테스트 `JpaIntroTest`(`@DataJpaTest`) — save/findById/쿼리 메서드를 SQL 없이 검증.
...
- **우리가 SQL을 한 줄도 안 썼는데 저장·조회가 됐다는 것** 자체가 "JPA가 SQL을 대신 생성"의 증거.
  (show-sql 로그에서 실제 생성된 insert/select를 눈으로도 확인 가능.)
