...
### 1-0. 빈(Bean)이란 무엇인가 — 어원과 의미
- **빈(Bean) = "Spring 컨테이너가 만들고 관리하는 객체"**다.
- **어원**: 1990년대 자바의 '재사용 가능한 소프트웨어 부품'을 **JavaBean**이라 불렀다(커피 산지 Java에
  빗대 '콩 = 부품' 느낌). Spring이 이 용어를 빌려 "컨테이너가 관리하는 객체"를 Bean이라 부른다.
- **그냥 `new`한 객체 ≠ Bean.** 같은 클래스라도 —
  - `new UserDao(...)` → 그냥 자바 객체(내가 만들고 내가 관리).
  - 컨테이너가 만들어 관리하는 UserDao → **Bean**(Spring이 생성·DI·싱글톤·생명주기를 책임).
- Bean이 되면 얻는 것: ① 컨테이너가 생성 ② 의존성 자동 주입(DI) ③ 싱글톤 관리 ④ 다른 Bean이 가져다 씀.
- 등록 방법: 여기선 `@Configuration` 클래스의 `@Bean` 메서드. (이후 PART에서 `@Component`/`@Service`/
  `@Repository`로 클래스에 직접 붙여 자동 등록하는 방식도 배운다.)
...
### ★ 헷갈리는 지점 — connectionMaker는 userDao만 쓰는데 왜 거기도 @Bean을 붙이나?
좋은 의문이다. 사실 **@Bean을 안 붙이고 `new NConnectionMaker()`를 userDao 안에서 직접 만들어도 동작은 한다.**
그런데 별도 `@Bean` 메서드로 빼고 @Bean을 붙이는 게 정석이고, 이유가 셋 있다.

1. **싱글톤 보장 (가장 중요).** @Bean을 붙이면 `connectionMaker()`를 코드상 여러 번 호출해도 Spring이
   **객체를 딱 하나만** 만들어 반환한다(싱글톤). 예를 들어 DAO가 늘어나면:
   ```java
   @Bean public UserDao  userDao()  { return new UserDao(connectionMaker()); }   // connectionMaker() 호출
   @Bean public OrderDao orderDao() { return new OrderDao(connectionMaker()); }  // 또 호출
   ```
   `connectionMaker()`가 두 번 불리지만 **ConnectionMaker 객체는 1개**가 만들어져 둘 다에 같은 게 주입된다.
   만약 @Bean이 없었다면 호출할 때마다 `new`가 실행돼 **매번 새 객체**가 생긴다(커넥션 풀 등을 공유 못 함).
   - 원리: `@Configuration` 클래스는 Spring이 **프록시로 감싸서**, @Bean 메서드 호출을 가로채 "이미 만든
     빈이 있으면 그것을 반환"하게 만든다. 그래서 `connectionMaker()`를 100번 불러도 객체는 하나다.
2. **다른 빈들이 공유·재사용.** Bean으로 등록해두면 지금은 userDao만 쓰지만, 나중에 생기는 여러 DAO가
   **같은 connectionMaker 빈**을 받아 쓸 수 있고, `getBean(ConnectionMaker.class)`로 단독으로 꺼내거나 교체·테스트하기도 쉽다.
3. **일관된 관리.** 모든 부품을 컨테이너가 관리하는 빈으로 통일하면, 설정·교체·확인이 한 곳(컨테이너)에서
   일관되게 된다. "어떤 건 빈, 어떤 건 그냥 new"가 섞이면 관리가 어렵다.

> 요약: "지금 userDao만 쓰는데?"는 맞는 관찰이지만, **@Bean으로 등록해야 싱글톤·공유·일관 관리가 보장**된다.
> 특히 1번(싱글톤) 때문에, 직접 new 하지 않고 @Bean 메서드로 빼서 호출하는 것이다.

### 싱글톤 레지스트리 — 빈은 기본 한 개
`@Bean` 메서드가 코드상 여러 번 불릴 것 같지만, 컨테이너는 각 빈을 **딱 하나만** 만들어 재사용한다.
그래서 `getBean("userDao")`를 100번 호출해도 **항상 같은 인스턴스**다.

#### ★ 헷갈리는 지점 — "stateless라서 공유한다"가 아니다 (인과를 거꾸로 읽기 쉬움)
"빈은 stateless 서비스라 하나를 공유한다"는 말은 인과가 헷갈린다. 정확히는 **두 개의 별개 이야기**다.
- **공유(싱글톤)하는 이유 = 효율.** DAO·Service는 보통 '로직만' 있고 들고 다닐 데이터가 없어서, 요청마다
  새로 `new` 할 이유가 없다. 하나만 만들어 모두가 재사용하면 생성·메모리 비용이 절약된다.
- **stateless = 그 공유를 '안전하게' 만드는 조건.** 하나를 여럿이 공유하면 동시성 문제(PART 7)가 걱정되는데,
  공유 객체에 **변하는 상태(인스턴스 필드)가 없으면** 여러 스레드가 동시에 메서드를 불러도 서로 건드릴 게
  없어 안전하다.

즉 순서는 이렇다: **효율을 위해 공유하고 싶다 → 공유하면 동시성 위험이 있다 → 그러니 상태를 두지 말자
(stateless) → 그래야 공유해도 안전하다.** stateless는 "공유하는 이유"가 아니라 "공유를 가능케 하는 전제"다.

여기서 **상태(state) = 객체가 들고 있는 '변하는 데이터'(인스턴스 필드)**를 뜻한다.
```java
// stateless (안전) — 요청 데이터를 필드에 저장하지 않고, 지역변수로만 다룸(지역변수는 스레드별 Stack = 7.1)
class UserService {
    private final UserDao userDao;         // 의존 부품(생성 후 안 바뀜)은 '상태'가 아니다
    User find(String id) { return userDao.get(id); }   // 받아서 바로 반환, 필드에 안 담음
}
// stateful (공유하면 위험) — 요청 데이터를 인스턴스 필드에 저장 -> 다른 요청이 덮어씀
class UserService {
    private String currentId;              // ← 변하는 상태
    User find(String id) { this.currentId = id; return userDao.get(this.currentId); }
}
```
비유: 계산기는 아무것도 기억하지 않아(stateless) 100명이 한 대를 돌려 써도 안 섞인다 → 공유 OK. 반면
개인 사물함처럼 내 물건을 보관하는 것(stateful)은 공유하면 남의 것과 섞인다 → 공유 불가.
...
- **Q. 빈(Bean)이란 무엇이고 어원은?**
  - 내 답: Spring 컨테이너가 만들고 관리하는 객체. 그냥 new한 객체와 달리 컨테이너가 생성·DI·싱글톤을
    책임진다. 어원은 자바의 재사용 부품 'JavaBean'(콩=부품). (1-0)

- **Q. connectionMaker가 userDao에서만 쓰이는데도 @Bean을 붙이는 이유는?**
  - 내 답: ①싱글톤 보장 — @Bean이면 여러 번 호출해도 객체 1개(없으면 호출마다 new), ②다른 빈들이
    공유·재사용 가능, ③컨테이너의 일관된 관리. 특히 싱글톤 때문에 직접 new 대신 @Bean 메서드로 뺀다.

- **Q. "stateless라서 공유한다"가 왜 틀린 표현인가?**
  - 내 답: 인과가 반대다. 공유(싱글톤)하는 이유는 '효율'(매번 new 안 함)이고, stateless는 그 공유를
    '안전하게' 만드는 조건이다. 공유 객체에 변하는 상태(필드)가 없어야 여러 스레드가 동시에 써도 안 섞인다.
    즉 stateless는 공유의 '이유'가 아니라 '전제'다.
