...
> ★ 동시성 문제는 '어디서' 생기나 — 공유되는 가변 상태에서만 생긴다.
> - **지역 변수에서는 발생하지 않는다.** 지역 변수는 스레드마다 각자의 '스택'에 따로 할당되기 때문이다
>   (PART 7.3 — 스레드마다 스택이 별도). 그래서 메서드 안에서 선언해 쓰는 값은 스레드끼리 절대 안 섞인다.
> - **발생하는 곳**: 여러 스레드가 함께 보는 '공유 가변 상태' — 같은 인스턴스의 필드(특히 싱글톤에서 자주),
>   또는 `static` 같은 공용 필드에 접근할 때. (위 traceId/depth가 바로 '싱글톤의 공유 필드'라 터진 것.)
> - **읽기만 하면 발생하지 않는다.** 동시성 문제는 어디선가 그 값을 '변경(쓰기)'하기 때문에 생긴다. 그래서
>   불변(immutable)·무상태(stateless)면 공유해도 안전하다(PART 8.5 "싱글톤은 무상태여야 한다"의 이유).
> 정리: **"공유 + 가변 + 쓰기"가 셋 다 모일 때만** 동시성 문제다. 하나라도 빠지면(지역 변수=비공유, 불변=불변,
> 읽기 전용=쓰기 없음) 안전하다. ThreadLocal은 이 중 '공유'를 깨서(스레드별 분리) 문제를 없애는 접근이다.
...
### 1-6. 실무에서 ThreadLocal이 쓰이는 곳 (Spring 내부)
ThreadLocal은 "요청을 처리하는 한 스레드 안의 여러 계층(컨트롤러·서비스·DAO)이, 인자로 일일이 넘기지 않고
'현재 요청의 문맥(context)'을 공유"할 때 쓴다. Spring 곳곳이 이 메커니즘 위에 있다.
- **SecurityContextHolder**(Spring Security): 현재 로그인 사용자/권한을 ThreadLocal에 보관 → 어느 계층에서든 꺼내 씀.
- **RequestContextHolder**(Spring MVC): 현재 HTTP 요청·세션 정보를 ThreadLocal로 보관.
- **TransactionSynchronizationManager**: 현재 트랜잭션·커넥션을 ThreadLocal에 바인딩(@Transactional의 토대 — 아래).
- **MDC**(로깅, SLF4J): 요청별 traceId를 ThreadLocal에 담아 모든 로그 줄에 자동으로 찍히게 함(이 단원의 로그 추적기와 같은 발상).
- 공통점: 모두 "요청 = 스레드 하나"라는 전제 위에서, 그 스레드에 문맥을 매달아 둔다. 그래서 1-3~1-5의 함정
  (remove로 정리, 비동기/풀에서 전파 한계)이 이들 모두에 똑같이 적용된다.
...
- **Q. 동시성 문제는 어디서 생기나? 지역 변수도 위험한가?**
  - 내 답: '공유 + 가변 + 쓰기'가 셋 다 모일 때만 생긴다. 지역 변수는 스레드마다 스택이 따로라(PART 7.3)
    안전하고, 위험한 건 같은 인스턴스의 필드(특히 싱글톤)나 static 공용 필드다. 읽기만 하면(불변/무상태)
    공유해도 안전하다. ThreadLocal은 이 중 '공유'를 깨서(스레드별 분리) 문제를 없앤다.

- **Q. 실무에서 ThreadLocal은 어디에 쓰이나?**
  - 내 답: SecurityContextHolder(로그인 사용자), RequestContextHolder(현재 요청),
    TransactionSynchronizationManager(현재 트랜잭션/커넥션), MDC(로그 traceId). 모두 "요청=스레드 하나"에
    문맥을 매달아 계층 간 공유한다. 따라서 remove 정리·비동기 전파 한계도 똑같이 적용된다.
