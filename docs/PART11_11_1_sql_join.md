...
#### ★ 헷갈리는 지점 — UNION ALL은 왜 4건이 나오나?
"개발 직원은 김개발·최개발 둘뿐인데 왜 UNION ALL은 4건이지?"가 가장 흔한 의문이다. 핵심은 **UNION이
'사람을 거르는 것'이 아니라 '독립적인 두 조회의 결과 줄을 위아래로 쌓는 것'** 이라는 점이다.
```
조회A: where dept_id = 1          ->  김개발, 최개발   (2줄)
조회B: where name like '%개발%'    ->  김개발, 최개발   (2줄)
```
- 같은 김개발이라도 **조회A가 만든 1줄**과 **조회B가 만든 1줄**은 서로 다른 줄이다(각 조회가 따로 뽑았으니까).
- **UNION ALL** = 두 결과를 그대로 이어 붙임(중복 검사 안 함):
  ```
  김개발  ← 조회A
  최개발  ← 조회A
  김개발  ← 조회B   (조회A의 김개발과는 별개 줄)
  최개발  ← 조회B
  => 4줄
  ```
- **UNION** = 위 4줄을 쌓은 뒤 '똑같은 줄'을 합침(중복 제거) → **2줄**.
- 그래서 **두 조회 결과가 겹칠 때만** 개수가 갈린다. 만약 두 조회가 하나도 안 겹쳤다면 UNION과 UNION ALL의
  결과 수는 같다.
- 비유: 두 사람이 각자 참석자 명단을 적어 왔다(A 명단: 김개발·최개발 / B 명단: 김개발·최개발).
  UNION ALL = 두 명단을 그냥 이어 붙임(이름 4개), UNION = 합치되 중복 이름은 한 번만(이름 2개).

#### UNION이 말하는 '똑같은 줄(중복)'의 기준
"중복을 제거한다"의 '중복'이 정확히 무엇인지 정리한다.
- **기준 = 선택한 모든 컬럼의 '값'이 행 단위로 전부 같을 때.** 컬럼이 여러 개면 **전부** 같아야 중복이다.
  하나라도 다르면 다른 줄로 본다.
  ```
  emp    | dept
  김개발 | 개발     ┐ emp·dept 둘 다 같음 -> 중복(1줄로 합침)
  김개발 | 개발     ┘
  김개발 | 영업     <- emp는 같아도 dept가 다름 -> '다른 줄'(안 합침)
  ```
  (앞 예제가 name 하나만 같으면 합쳐진 건, 선택 컬럼이 name '하나뿐'이라서다.)
- **컬럼 이름은 무관하다.** 중복 판정은 컬럼 이름이 아니라 '위치(순서)별 값'으로 한다. 그래서
  `select name ... union select title ...`처럼 두 SELECT의 컬럼명이 달라도 된다. 결과 컬럼 이름은 **첫 번째
  SELECT 것**을 따라간다.
- **컬럼 개수·타입 일치는 '전제 조건'**이지 중복 기준이 아니다. 안 맞으면 UNION 자체가 에러다(세로로 쌓으려면 모양이 맞아야 하니까).
- ★ **NULL 주의**: 보통 SQL에서 `NULL = NULL`은 '참'이 아니라 '알 수 없음'이다. 그런데 **UNION의 중복 판정에서만은
  NULL끼리도 '같다'고 보고 합친다.** (예: `NULL|인사` 두 줄은 UNION에서 1줄로 합쳐짐. 일반 `WHERE a=b` 비교였다면 안 합쳐졌을 것.)
...
조회A = `where dept_id = 1`(김개발·최개발), 조회B = `where name like '%개발%'`(김개발·최개발).
**각 조회가 독립적으로** 김개발·최개발을 뽑으므로, 합치기 전 '원재료'는 이미 4줄이다:

| name | 어느 조회에서 |
|---|---|
| 김개발 | 조회A |
| 최개발 | 조회A |
| 김개발 | 조회B (조회A의 김개발과 별개 줄) |
| 최개발 | 조회B |

UNION ALL (위 4줄을 그대로 둠):
...
→ **4건**. UNION ALL은 중복 검사를 안 하니 두 조회 결과가 그대로 쌓인다.
...
→ **2건**. 똑같은 줄을 하나로 합친다. (두 조회가 겹칠 때만 UNION과 UNION ALL 개수가 달라진다.)
...
- **Q. UNION이 중복으로 보는 '똑같은 줄'의 기준은?**
  - 내 답: 선택한 모든 컬럼의 값이 행 단위로 전부 같을 때(여러 컬럼이면 전부 일치, 하나라도 다르면 다른 줄).
    컬럼 이름은 무관(위치별 값으로 비교, 결과 컬럼명은 첫 SELECT 것). 컬럼 수·타입 일치는 중복 기준이 아니라
    UNION 실행의 전제. NULL은 일반 비교와 달리 UNION 중복 판정에선 같은 값으로 취급해 합친다.
