...
#### ★ BPP는 메서드가 두 개다 — 그리고 프록시는 '초기화 후(after)'에 교체된다
BeanPostProcessor에는 두 메서드가 있고, 빈 생명주기에서 호출 순서가 정해져 있다.
- `postProcessBeforeInitialization` — 초기화 콜백(@PostConstruct) **전**.
- `postProcessAfterInitialization` — 초기화 콜백 **후**. ← 자동 프록시 생성기는 **여기서** 프록시로 교체한다.
```
빈 생성(생성자) -> 의존성 주입 -> BPP.before -> @PostConstruct(초기화) -> BPP.after(여기서 프록시 교체) -> 사용
```
- 왜 after인가: @PostConstruct 같은 초기화까지 끝난 **'완성된 원본'을 감싸야** 하기 때문이다. 그래서 프록시는
  초기화 '후'에 만들어진다.
- ★ **13.4 함정의 근거**: 뒤집어 보면, **@PostConstruct가 실행되는 시점엔 아직 프록시가 없다**(프록시는 그 다음
  after에 생김). 그래서 13.4에서 "@PostConstruct + @Transactional은 트랜잭션이 안 걸린다"가 나온다 — 12.8의
  이 순서가 그 직접적 뿌리다. (실측 예제4에서 생성→before→@PostConstruct→after 순서를 눈으로 확인.)

#### ★ BPP는 '프록시 전용'이 아니라 스프링의 범용 확장점
BeanPostProcessor를 "프록시 바꿔치기 도구"로만 보기 쉽지만, 사실 **스프링 어노테이션 처리의 공통 메커니즘**이다.
- `@Autowired` 주입 → `AutowiredAnnotationBeanPostProcessor`
- `@PostConstruct`/`@PreDestroy` → `CommonAnnotationBeanPostProcessor`(InitDestroyAnnotationBeanPostProcessor)
- `@Async`·`@Transactional` 등 프록시 → 자동 프록시 생성기(역시 BPP)
- 즉 "스프링이 빈에 뭔가를 자동으로 해주는 마법"의 상당수가 BPP다. 자동 프록시 생성기는 그 한 종류일 뿐이다.
> ★ 참고: BPP는 자기가 남을 후처리해야 하므로 **일반 빈보다 먼저** 인스턴스화된다. 또 자동 프록시 생성기는
> 이미 프록시인 빈을 또 감싸지 않는다(중복 프록시 방지).
...
- `Example4_BeanLifecycleOrder` — BPP.before/@PostConstruct/BPP.after 호출 순서 확인(프록시는 after).
...
./gradlew runStage -Pmain=com.study.part12_aop.s08_bean_postprocessor.Example4_BeanLifecycleOrder
```
...
1) 생성자 (객체 생성)
2) BPP.before (초기화 '전')
3) @PostConstruct (초기화)        <- 이 시점엔 아직 프록시가 없다
4) BPP.after (초기화 '후')         <- 자동 프록시 생성기라면 여기서 프록시 교체
```
- 생성→before→@PostConstruct→after 순서 확인. ✅ → 프록시는 after에 생기므로 @PostConstruct 시점엔 프록시가
  없다 = 13.4 @PostConstruct+@Transactional 함정의 직접적 근거.
...
- **Q. BPP의 두 메서드와, 프록시 교체가 'after'인 이유는? 13.4와 어떻게 연결되나?**
  - 내 답: before(초기화 전)/after(초기화 후) 두 메서드가 있고, 프록시 교체는 after에서 한다(@PostConstruct까지
    끝난 완성된 원본을 감싸야 하므로). 순서는 생성→주입→before→@PostConstruct→after. 그래서 @PostConstruct
    시점엔 아직 프록시가 없어 @Transactional이 안 걸린다(13.4 함정의 뿌리).

- **Q. BeanPostProcessor는 프록시 전용인가?**
  - 내 답: 아니다. 스프링 어노테이션 처리의 범용 확장점이다. @Autowired(AutowiredAnnotationBeanPostProcessor),
    @PostConstruct(CommonAnnotationBeanPostProcessor), @Transactional/@Async 프록시(자동 프록시 생성기) 등이
    모두 BPP다. 자동 프록시 생성기는 그중 한 종류일 뿐.
