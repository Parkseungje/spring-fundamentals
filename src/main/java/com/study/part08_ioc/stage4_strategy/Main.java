...
/**
 * [STAGE 4 데모] UserDao는 그대로 두고, 주입하는 ConnectionMaker만 바꿔 끼운다.
 *
 ...
 * 단, 아직 'new NConnectionMaker()'와 조립(주입)을 Main이 직접 한다 -> stage5에서 컨테이너에 위임.
 */
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        dao.createTable();
        dao.add(new User("s4", "스프링준비"));
        System.out.println("[stage4] " + dao.get("s4"));
        System.out.println("=> UserDao는 ConnectionMaker(인터페이스)에만 의존. 구현 교체에도 UserDao 수정 0(OCP/DIP).");
        System.out.println("   하지만 'new NConnectionMaker()'와 주입을 아직 Main이 직접 함 -> stage5: 컨테이너(IoC).");
    }
}
